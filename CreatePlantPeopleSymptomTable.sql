...
/****** Object:  StoredProcedure [dbo].[CreatePlantPeopleSymptomTable]    Script Date: 5/4/2018 3:25:21 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[CreatePlantPeopleSymptomTable]
AS
BEGIN 
    -- Create tables
	IF OBJECT_ID('dbo.PlantPeopleSymptom','U') IS NOT NULL
		DROP TABLE dbo.PlantPeopleSymptom;
	
	CREATE TABLE PlantPeopleSymptom(
		PlantPeopleSymptomID int IDENTITY(1,1),
		ToxicPlantID int not null,
		PeopleSymptomID int,
		primary key (PlantPeopleSymptomID)
	)

	IF OBJECT_ID('dbo.TmpTable','U') IS NOT NULL
	DROP TABLE dbo.TmpTable;

	SELECT ToxicPlantID, SymptomsInpeople 
	INTO TmpTable
	FROM ToxicPlant;
	...
	BEGIN
		SELECT TOP 1 @SymptomValue = SymptomsInpeople,
		...
		BEGIN
			...
			PRINT @singleSymtom;
			SELECT @matchID = SymptomID FROM PeopleSymptom WHERE symptoms = @singleSymtom;
			PRINT @matchID;
			INSERT INTO PlantPeopleSymptom(ToxicPlantID, PeopleSymptomID) VALUES(@tmpToxicPlantID, @matchID);
